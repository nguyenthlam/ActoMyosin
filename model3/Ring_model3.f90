...
!   Select job:
   call setjob(nthreads,jobid,mbrad,mwid,rrad,rwid,rthick,falen,fanum,crlknum1,crlknum2, &
        myonum,nnode_a,node_size,kmemb,tension1,tension2,crlkorient,j_dep,cofilin,jxldel,jmyoturn,jxlturnover,halftime)
...
!   allocate(xwall(nwall),ywall(nwall),zwall(nwall))
...
17    call random_number(r)
      ...
      phi_node_a(n)=n*dphi+dphi*(r-0.5d0)/2

      call random_number(r)

      xnode_a(n)=rwid*(r-0.5d0)/2

      ynode_a(n)=mbrad*cos(phi_node_a(n))

      znode_a(n)=mbrad*sin(phi_node_a(n))

      jm=0

      do nm=1,nmemb

         if(jm==nmb2node_a)then
            exit
         end if

         if(mark(nm)==1) cycle

         dx=xnode_a(n)-xmemb(nm)
         dy=ynode_a(n)-ymemb(nm)
         dz=znode_a(n)-zmemb(nm)
...
!  node association:
   allocate(a2node(fanum))

!  bonds:
   allocate(bondfa(2,nfa))
...
!      x0=-rwid/2+rwid*r
      ...
      x0=xnode_a(jnode)+node_size*(r-0.5d0)
...
!      call random_number(r)
      ...
      phi0=phi_node_a(jnode)!(nnode*r+1)

      call random_number(r)

      phi0=phi0+phi_node_size*(r-0.5d0)
...
13    call random_number(r)
      ...
      xmyo(nmyo)=-rwid/2+rwid*r!xnode_my(jnode)+node_size*(r-0.5d0)/2

!     pick the angle of first body bead:

      call random_number(r)

      phi0=2*pi*r!phi_node_my(jnode)+phi_node_size*(r-0.5d0)/2

!     pick the "radius" of the myosin 1st body bead

      call random_number(r)

      rad=mbrad-l_mem*r!-node_size*r/4
...
!     binding to membrane:
      ...
      do jm=1,nmemb
         ...
         dx=xmyo(nmyo)-xmemb(jm)
         dy=ymyo(nmyo)-ymemb(jm)
         dz=zmyo(nmyo)-zmemb(jm)
...
!     direction of the myosin filament:
      ...
      dy=y0-ymyo(nmyo)!ynode_my(jnode)
      dz=z0-zmyo(nmyo)!znode_my(jnode)
...
!  write out the ring:
   ...
   call makering(nbondwal,nwall,nmemb,nnode_a,nfa1,nfa2,fanum1,fanum2,nmyo,nbondmyo,myonum,nlk, &
...
!  getting the initial ring:
   ...
   call getinfo(nstart,jfile,time0,runtime,natom,natom_a1,natom_a2,natom_a2n,nwall,nxsol,nphisol, &
...
!  nodes haboring f-actin
   allocate(a2node(fanummax))

!  binding myosin to membrane
   allocate(my2mem(myonum))
...
!  crosslinker type to tell if it is deleted:
   ...
   call ringin(nstart,nxsol,nphisol,nmemb,fanum,nfa,myonum,nmyo,nnode_a,mybodylen,crlknum, &
...
!  forces:
   ...
   allocate(fxnode_a(nnode_a),fynode_a(nnode_a),fznode_a(nnode_a))
...
!  to prevent tethers from slipping past each other:
   allocate(pairnode_aa(2,fanummax*100))
...
!------------------------------------------------------------
   ...
   nframe=0
         ...
         call writedcd(nframe,junit,natom,natom_a1,natom_a2,natom_a2n,nxsol,nphisol,nmemb,nnode_a, &
...
!        add new F-actin:
         ...
         if(nmono>falen.and.fanum1<fanummax1)then
            ...
            call newactin(myonum,crlknum,nmono,nxsol,nphisol,falen,nnode_a,nmb2node_a,jdir,fanum,fanum1,fanum2, &
               nfa,nfa1,nfa2,jsursol,memb2node_a,apar,jfasol,astart,alen,fadist,apos,filid,a2node, &                fa2myo,fa2lk,l_a,l_mem,pi,lnode,xfa,yfa,zfa,xmemb,ymemb,zmemb, &
...
!        depolymerization of F-actin:
         ...
         if(j_dep==1)then
            call depoly(nfa,nfa1,nfa2,fanum,fanum1,fanum2,apar,apos,filid,fadist,jfasol,astart,alen,a2node, &
...
!        myosin turnover:
         ...
         if(jmyoturn==1)then
            ...
            call myoturnover(nmyoturn,nnode_a,nmb2node_a,nmemb,myonum,mybodylen,nxsol,nphisol,mytyp,jmysol,mybody, &
               myhead,jsursol,jfasol,memb2node_a,mydist,my2mem,l_mb,l_mem,pi,pmyturn,dt,dphisol,xrmin,xrmax, &
...
!        non-bond interaction: exclusion effect and coulomb
         ...
         call tethering(nnode_a,nmb2node_a,memb2node_a,fanum, &
              astart,a2node,myonum,mybody,my2mem,fxnode_a,fynode_a,fznode_a, &
              fxmemb,fymemb,fzmemb,fxfarep,fyfarep,fzfarep,fxmyorep,fymyorep,fzmyorep, &
              xnode_a,ynode_a,znode_a,xmemb,ymemb,zmemb, &
              xfa,yfa,zfa,xmyo,ymyo,zmyo,knode,lnode,lnode2,kmy2mem,lmy2mem, &
...
!     update coordinates:
      ...
      if(jforce1==1)then
         ...
         fmaxnode_a2=maxval(fxnode_a*fxnode_a+fynode_a*fynode_a+fznode_a*fznode_a)

         fmax2=max(fmaxmb2,fmaxfa2,fmaxmyo2,fmaxlk2,fmaxnode_a2)
      ...
      xnode_a=xnode_a+gam*fxnode_a
      ynode_a=ynode_a+gam*fynode_a
      znode_a=znode_a+gam*fznode_a
...
!------------------------------------------------
      ...
      if(runtime-oldtime>printtime)then
         ...
         if(nframe>=200)then
            ...
            call ringout(nstart,natom,natom_a1,natom_a2,natom_a2n,nwall,nxsol,nphisol,nmemb,nnode_a,nact1,nact2, &
