...
!==================================================
   ...
   subroutine setjob(nthreads,jobid,mbrad,mwid,rrad,rwid,rthick,falen,fanum,crlknum1,crlknum2, &
              myonum,nnode,node_size,kmemb,tension1,tension2,crlkorient,j_dep,cofilin,jxldel,jmyoturn,jxlturnover,halftime)

   implicit none

   integer nthreads,jobid,fanum,falen,crlknum1,crlknum2,nnode,myonum
...
!=========================================================
   ...
   subroutine makering(nbondwal,nwall,nmemb,nnode_a,nfa1,nfa2,fanum1,fanum2,nmyo,nbondmyo,myonum,nlk, &
              nbondlk,mybodylen,crlknum1,crlknum2,nxsol,nphisol,nmb2node_a,astart,alen, &
              a2node,my2mem,filid,apos,lkstart,bondwal,bondmyo,mybody,myhead,bondlk,memb2node_a, &
              ...
              xnode_a,ynode_a,znode_a)



   implicit none

   integer,value::nbondwal,nwall,nmemb,nnode_a,nfa1,nfa2,fanum1,fanum2,nmyo,nbondmyo,myonum
   integer,value::nlk,nbondlk,mybodylen,crlknum1,crlknum2,nxsol,nphisol,nmb2node_a
   integer nfa,fanum,nact1,nact2,nfamax1,nfamax2,natom,natom_a1,natom_a2,nbond,nbond_a1,nbond_a2
   integer fanummax,fanummax1,fanummax2,natom_a2n,nbond_a2n,crlknum,crlknummax,crlknumactive,nbond0

   integer jstart,na,nmyoturn
   integer jm,nm,j
   integer izero,ires,n,jatom
   integer j1,j2,j3,j4,j5,j6,j7,j8,nline,i,jx,jp
   integer nf,ja,jnode,jmb

   integer,allocatable,dimension(:),intent(in)::astart,alen,a2node,my2mem,filid,apos,lkstart
   integer,allocatable,dimension(:,:),intent(in)::bondwal,bondmyo,mybody,myhead,bondlk,memb2node_a
   ...
   double precision,allocatable,dimension(:),intent(in)::xnode_a,ynode_a,znode_a
!   double precision,allocatable,dimension(:),intent(in)::xnode_my,ynode_my,znode_my
...
!  number of beads to start:
   natom=nwall+nmemb+nnode_a!+nnode_my!+2*nbondfa+nmyo+nlk+nnode
...
!  to visualize tethering of F-actin to nodes:
...
!  to visualize tethering of myosin to membrane:
...
!  for actin nodes:
   ...
   do n=1,nnode_a
      ...
      write(2,42)tex,jatom,typ,res,ires,xnode_a(n)/10,ynode_a(n)/10,znode_a(n)/10,w1,w2,segid
...
!      write(2,42)tex,jatom,typ,res,ires,xnode_my(n)/10,ynode_my(n)/10,znode_my(n)/10,w1,w2,segid
...
!  binding of actin plus ends to nodes:
   ...
   do nf=1,fanum
      ...
      write(2,42)tex,jatom,typ,res,ires,xnode_a(jnode)/10,ynode_a(jnode)/10,znode_a(jnode)/10,w1,w2,segid
   ...
   if(nbond<nbond_a2n)then

      nbond0=nbond

      do n=nbond0+1,nbond_a2n
...
!   write(1)rwall(1:nphisol,1:nxsol)
   ...
   write(1)nnode_a
   write(1)xnode_a(1:nnode_a)
   write(1)ynode_a(1:nnode_a)
   write(1)znode_a(1:nnode_a)

!   write(1)nnode_my
!   write(1)xnode_my(1:nnode_my)
!   write(1)ynode_my(1:nnode_my)
!   write(1)znode_my(1:nnode_my)
...
!  write configuration
   ...
   write(1,*)natom,natom_a1,natom_a2,natom_a2n
...
!   write(1,*)xboundmin,xboundmax
   ...
   write(1,*)nnode_a
   do n=1,nnode_a
      write(1,*)memb2node_a(1:nmb2node_a,n)
   end do
   write(1,*)
...
!   write(1,*)nnode_my
!   do n=1,nnode_my
!      write(1,*)memb2node_my(1:nmb2node_my,n)
!   end do
!   write(1,*)
...
!   deallocate(fa1stbound)
   ...
   do n=1,myonum
      write(1,*)my2mem(n),myhead(1:2,n),mybody(1:mybodylen,n)
...
!=========================================================
   subroutine writedcd(nframe,junit,natom,natom_a1,natom_a2,natom_a2n,nxsol,nphisol,nmemb,nnode_a, &
              fanum1,fanum2,nmyo,myonum,nlk,alen,astart,a2node,my2mem,mybody,xfa,yfa,zfa,xmyo, &
              ymyo,zmyo,xnode_a,ynode_a,znode_a, &
   ...
   integer nframe
   integer,value::junit,natom,natom_a1,natom_a2,natom_a2n,nxsol,nphisol,nmemb,nnode_a,fanum1,fanum2,nmyo,myonum,nlk
   integer n,j,ja,jw,jm,jp,jx,jstart,jnode,jmemb

   integer,allocatable,intent(in),dimension(:)::alen,astart,a2node,my2mem
   ...
   double precision,allocatable,intent(in),dimension(:)::xnode_a,ynode_a,znode_a!,xnode_my,ynode_my,znode_my
...
!   nnode=nnode_a+nnode_my
   ...
   xw(jw+1:jw+nnode_a)=0.1*xnode_a(1:nnode_a)
   yw(jw+1:jw+nnode_a)=0.1*ynode_a(1:nnode_a)
   zw(jw+1:jw+nnode_a)=0.1*znode_a(1:nnode_a)

   jw=jw+nnode_a

!   xw(jw+1:jw+nnode_my)=0.1*xnode_my(1:nnode_my)
!   yw(jw+1:jw+nnode_my)=0.1*ynode_my(1:nnode_my)
!   zw(jw+1:jw+nnode_my)=0.1*znode_my(1:nnode_my)
...
!  tethering actin to nodes:
   ...
   do n=1,fanum1+fanum2
      ...
      xw(jw)=0.1*xnode_a(jnode)
      yw(jw)=0.1*ynode_a(jnode)
      zw(jw)=0.1*znode_a(jnode)
   ...
   if(jw<natom_a2n)then

      xw(jw+1:natom_a2n)=0.0
      yw(jw+1:natom_a2n)=0.0
      zw(jw+1:natom_a2n)=0.0
...
!=========================================================
   ...
   subroutine getinfo(nstart,jfile,time0,runtime,natom,natom_a1,natom_a2,natom_a2n,nwall,nxsol,nphisol, &
   ...
   integer nstart,jfile,natom,natom_a1,natom_a2,natom_a2n,nwall,nxsol,nphisol
   integer nmemb,nact1,nact2,nfamax1,nfamax2,nfa1,nfa2,fanummax1,fanummax2,fanum1,fanum2
...
21 read(1,*)chara
   ...
   read(1,*)natom,natom_a1,natom_a2,natom_a2n
...
!=========================================================
   ...
   subroutine ringin(nstart,nxsol,nphisol,nmemb,fanum,nfa,myonum,nmyo,nnode_a,mybodylen,crlknum, &
               nlk,nmb2node_a,astart,alen,a2node,my2mem,filid,apos,lkstart,lktyp,mybody,myhead,mytyp,apar, &
                memb2node_a,fa2myo,fa2lk,xboundmin,xboundmax,dxsol,dphisol,xwall,ywall,zwall, &
   ...
   integer,value::nstart,nxsol,nphisol,nmemb,fanum,nfa,myonum,nmyo,nnode_a
   integer,value::mybodylen,crlknum,nlk,nmb2node_a
   integer n,jread(10)

   integer,allocatable,dimension(:)::astart,alen,a2node,my2mem,filid,apos,lkstart,lktyp
   integer,allocatable,dimension(:,:)::mybody,myhead,mytyp,apar,memb2node_a
...
!   read(1)rwall(1:nphisol,1:nxsol)
   ...
   read(1)xnode_a(1:nnode_a)
   read(1)ynode_a(1:nnode_a)
   read(1)znode_a(1:nnode_a)

!   read(1)jread(1)!nnode
!   read(1)xnode_my(1:nnode_my)
!   read(1)ynode_my(1:nnode_my)
!   read(1)znode_my(1:nnode_my)
...
13 read(1,*)chara
   ...
   do n=1,nnode_a
      read(1,*)memb2node_a(1:nmb2node_a,n)
   end do
...
!   do n=1,nnode_my
!      read(1,*)memb2node_my(1:nmb2node_my,n)
!   end do
...
5  read(1,*)chara
   ...
   do n=1,myonum
      read(1,*)my2mem(n),myhead(1:2,n),mybody(1:mybodylen,n)
...
!=========================================================
   ...
   subroutine ringout(nstart,natom,natom_a1,natom_a2,natom_a2n,nwall,nxsol,nphisol,nmemb,nnode_a,nact1,nact2, &
               nfamax1,nfamax2,nfa,nfa1,nfa2,fanummax1,fanummax2,fanum,fanum1,fanum2,nmyo,myonum,nmyoturn, &
                mybodylen,nlk,crlknum1,crlknum2,crlknummax,crlknumactive, &
                nmb2node_a,astart,alen,a2node,my2mem,filid,apos,lkstart,lktyp,mybody,myhead,mytyp,fa2myo, &
                 fa2lk,apar,memb2node_a,dxsol,dphisol,xboundmin,xboundmax,xfa,yfa,zfa, &
                  xmyo,ymyo,zmyo,xnode_a,ynode_a,znode_a,xlk,ylk,zlk, &
   ...
   integer nstart
   integer,value::natom,natom_a1,natom_a2,natom_a2n,nwall,nxsol,nphisol,nmemb,nnode_a,nact1,nact2
   integer,value::nfamax1,nfamax2,nfa,nfa1,nfa2,fanummax1,fanummax2,fanum,fanum1,fanum2
   integer,value::nmyo,myonum,mybodylen,nmyoturn
   integer,value::nlk,crlknum1,crlknum2,crlknummax,crlknumactive,nmb2node_a

   integer n,crlknum

   integer,allocatable,intent(in),dimension(:)::astart,alen,a2node,my2mem,filid,apos,lkstart,lktyp
   integer,allocatable,intent(in),dimension(:,:)::mybody,myhead,mytyp,fa2myo,fa2lk,apar,memb2node_a
...
!$omp shared(nrand,jrand,crlknum,crlknum1,lklen,neinum,seed) &
...
!=========================================================
   ...
   subroutine tethering(nnode_a,nmb2node_a,memb2node_a,fanum, &
               astart,a2node,myonum,mybody,my2mem,fxnode_a,fynode_a,fznode_a, &
                fxmemb,fymemb,fzmemb,fxfa,fyfa,fzfa,fxmyo,fymyo,fzmyo, &
                 xnode_a,ynode_a,znode_a,xmemb,ymemb,zmemb,xfa,yfa,zfa,xmyo,ymyo,zmyo, &
                  knode,lnode,lnode2,kmy2mem,lmy2mem,npairnode_aa,pairnode_aa)

   implicit none

   integer,value::nnode_a,fanum,myonum,nmb2node_a,npairnode_aa
   integer jnode,jmb,ja,jmy,j,nf,nm,n1,n2,nf1,nf2,ja1,ja2,j1,j2,n

   integer,allocatable,dimension(:),intent(in)::a2node,my2mem,astart
   integer,allocatable,dimension(:,:),intent(in)::memb2node_a,mybody,pairnode_aa

   double precision,value::knode,lnode,lnode2,kmy2mem,lmy2mem
   double precision dx,dy,dz,f,fx,fy,fz,dist,dist2,radnode,rad,l2on
   ...
   double precision,allocatable,dimension(:),intent(in)::xnode_a,ynode_a,znode_a!,xnode_my,ynode_my,znode_my
   double precision,allocatable,dimension(:),intent(in)::xmemb,ymemb,zmemb
...
!$omp shared(nnode_a,fanum,myonum) &
!$omp private(jnode,jmb,ja,jmy,j,nf,nm) &

!$omp shared(a2node,my2mem,astart,nmb2node_a,memb2node_a,mybody,knode,lnode,lnode2,kmy2mem,lmy2mem,l2on) &
!$omp private(dx,dy,dz,f,fx,fy,fz,dist,dist2,radnode,rad,xno,yno,zno,xmb,ymb,zmb) &

!$omp shared(xmemb,ymemb,zmemb) &
!$omp shared(xnode_a,ynode_a,znode_a) &
!$omp shared(fxnode_a,fynode_a,fznode_a,fxmemb,fymemb,fzmemb) &
!$omp shared(xfa,yfa,zfa,xmyo,ymyo,zmyo,fxfa,fyfa,fzfa,fxmyo,fymyo,fzmyo) &

!$omp shared(npairnode_aa,pairnode_aa,l2max,lmax) &
!$omp private(n1,n2,nf1,nf2,ja1,ja2,j1,j2,n) &
   ...
   do jnode=1,nnode_a
      ...
      do j=1,nmb2node_a
         ...
         if(dist2>lnode2)then
            ...
            fxnode_a(jnode)=fxnode_a(jnode)-fx
            fynode_a(jnode)=fynode_a(jnode)-fy
            fznode_a(jnode)=fznode_a(jnode)-fz
...
!            fxnode_my(jnode)=fxnode_my(jnode)-fx
!            fynode_my(jnode)=fynode_my(jnode)-fy
!            fznode_my(jnode)=fznode_my(jnode)-fz
...
!$omp do schedule (guided,32)
   ...
   do nf=1,fanum
      ...
      dx=xnode_a(jnode)-xfa(ja)
      dy=ynode_a(jnode)-yfa(ja)
      dz=znode_a(jnode)-zfa(ja)
...
!$omp do schedule (guided,32)
   ...
   do nm=1,myonum
      ...
      dx=xmemb(jmb)-xmyo(jmy)
      dy=ymemb(jmb)-ymyo(jmy)
      dz=zmemb(jmb)-zmyo(jmy)
...
!$omp do
   ...
   do n=1,npairnode_aa
      ...
      dx1=0.1d0*(xfa(ja1)-xnode_a(n1))
      dy1=0.1d0*(yfa(ja1)-ynode_a(n1))
      dz1=0.1d0*(zfa(ja1)-znode_a(n1))

      dx2=0.1d0*(xfa(ja2)-xnode_a(n2))
      dy2=0.1d0*(yfa(ja2)-ynode_a(n2))
      dz2=0.1d0*(zfa(ja2)-znode_a(n2))

      d2min=l2max+1.0d0

      dx0=xnode_a(n1)-xnode_a(n2)
      dy0=ynode_a(n1)-ynode_a(n2)
      dz0=znode_a(n1)-znode_a(n2)
...
!$omp atomic
         fxnode_a(n1)=fxnode_a(n1)+fx
!$omp atomic
         fynode_a(n1)=fynode_a(n1)+fy
!$omp atomic
         fznode_a(n1)=fznode_a(n1)+fz

!$omp atomic
         fxnode_a(n2)=fxnode_a(n2)-fx
!$omp atomic
         fynode_a(n2)=fynode_a(n2)-fy
!$omp atomic
         fznode_a(n2)=fznode_a(n2)-fz

!$omp atomic
         fxfa(ja1)=fxfa(ja1)+fx
!$omp atomic
         fyfa(ja1)=fyfa(ja1)+fy
!$omp atomic
         fzfa(ja1)=fzfa(ja1)+fz

!$omp atomic
         fxfa(ja2)=fxfa(ja2)-fx
!$omp atomic
         fyfa(ja2)=fyfa(ja2)-fy
!$omp atomic
         fzfa(ja2)=fzfa(ja2)-fz
...
!=========================================================
   ...
   subroutine newactin(myonum,crlknum,nmono,nxsol,nphisol,falen,nnode_a,nmb2node_a,jdir,fanum,fanum1,fanum2, &
   ...
   integer,value::myonum,crlknum,nmono,nxsol,nphisol,falen,nnode_a,nmb2node_a,jdir
   integer fanum,fanum1,fanum2,nfa,nfa1,nfa2

   integer length,na,nnew,jm,jmax,jx0,jp0,j,jx,jp,jxget,jpget,j1,j2,n,jnode,nl,jl,nm

   integer,allocatable,dimension(:,:),intent(in)::jsursol,memb2node_a
   ...
   real(kind=8),value::l_a,l_mem,pi,lnode

   real(kind=8)::d2max,l_memby2,dshift,r,xn0,yn0,zn0,distance,x0,y0,z0,phi,rad,dphi
   real(kind=8)::dist2,d2,dx,dy,dz,xn,yn,zn,proj,proj0,psum!,distmin,distmax
...
!  pick a mother node:
   ...
   allocate(prob(nnode_a),ncount(nnode_a))
...
!   prob(1:nnode_a)=1.0d0/ncount(1:nnode_a)
   ...
   do n=1,nnode_a
      ...
      if(ncount(n)==0)then
         ...
         prob(n)=1.0d0/ncount(n)
      end if

   end do


   psum=sum(prob(1:nnode_a))

   prob(1:nnode_a)=prob(1:nnode_a)/psum
...
!=========================================================
   ...
   subroutine depoly(nfa,nfa1,nfa2,fanum,fanum1,fanum2,apar,apos,filid,fadist,jfasol,astart,alen,a2node, &
   ...
   integer,allocatable,dimension(:)::astart,alen,a2node,apos,filid,fadist,map
...
!=========================================================
   ...
   subroutine myoturnover(nmyoturn,nnode,nmb2node,nmemb,myonum,mybodylen,nxsol,nphisol,mytyp,jmysol,mybody, &
               myhead,jsursol,jfasol,memb2node,mydist,my2mem,l_mb,l_mem,pi,pmyturn,dt,dphisol,xrmin,xrmax, &
                lmy2mem,xmyo,ymyo,zmyo,xmemb,ymemb,zmemb,xnorsurf,ynorsurf,znorsurf,xsurf,ysurf,zsurf)


   implicit none

   integer,value::nnode,nmb2node,nmemb,myonum,mybodylen,nxsol,nphisol

   integer nmyoturn

   integer jb,jh,nm,jx0,jp0,jm,jx,jp,j,jxget,jpget,j1,j2,na,n

   integer,allocatable,dimension(:,:)::mytyp,jmysol
   integer,allocatable,dimension(:,:),intent(in)::mybody,myhead,jsursol,jfasol,memb2node
!   integer,allocatable,dimension(:),intent(in)::a2mem
   integer,allocatable,dimension(:)::mydist,my2mem
...
!   real(kind=8),value::xrmin,xrmax,distmin,distmax
   ...
   real(kind=8)::dx1,dy1,dz1,dx2,dy2,dz2,cosp,sinp
   real(kind=8)::xdir,ydir,zdir,invdist,distance

   real(kind=8),allocatable,dimension(:)::xmyo,ymyo,zmyo
   real(kind=8),allocatable,dimension(:),intent(in)::xmemb,ymemb,zmemb
   real(kind=8),allocatable,dimension(:,:),intent(in)::xnorsurf,ynorsurf,znorsurf,xsurf,ysurf,zsurf
...
!   do na=1,nfa
!      if(a2mem(na)>0) mark(a2mem(na))=1
!   end do

!$omp parallel &
!$omp default(none) &
!$omp private(jb,jh,nm,jx0,jp0,jm,jx,jp,j,jxget,jpget,j1,j2) &
!$omp private(dshift,r,xn0,yn0,zn0,x0,y0,z0,phi,rad) &
!$omp private(dist2,d2,dx,dy,dz,xn,yn,zn,proj,proj0,xmb,ymb,zmb) &
!$omp private(dx1,dy1,dz1,dx2,dy2,dz2,cosp,sinp) &
!$omp private(xdir,ydir,zdir,invdist,distance) &
!$omp shared(nmemb,myonum,mybodylen,nxsol,nphisol) &
!$omp shared(mytyp,jmysol) &
!$omp shared(mybody,myhead,jsursol,jfasol) &
!$omp shared(mydist,my2mem,mark) &
!$omp shared(l_mb,l_mem,pi,pmyturn,dt,dphisol,xrmin,xrmax,lmy2mem) &
!$omp shared(xmyo,ymyo,zmyo) &
...
!$omp do
   ...
   do nm=1,myonum
      ...
      if(mytyp(1,nm)/=2.or.mytyp(2,nm)/=2) cycle
...
!     now assign coordinates for the body:
      ...
      do j=1,mybodylen
         ...
         if(j==mybodylen)then
            ...
            jmysol(1,myhead(1:2,nm))=jx0

            jmysol(2,myhead(1:2,nm))=jp0
...
!============================================================================
   ...
   integer n1,n2,n3,n4,jexit,j1,j2,j3,j4,m1,m2,jp,np,nf1,nf2
   ...
   integer,allocatable,intent(in),dimension(:)::a2node

   integer,allocatable,dimension(:,:)::pair_myac,pair_lkac,pair_mylk,pair_ac2,pair_my2,pair_lk2
   integer,allocatable,dimension(:,:)::pair_mb2,pairpart,pairnode_aa
   ...
   double precision,allocatable,intent(in),dimension(:)::xfa,yfa,zfa,xmyo,ymyo,zmyo,xlk,ylk,zlk
   double precision,allocatable,intent(in),dimension(:)::xmemb,ymemb,zmemb,xnode_a,ynode_a,znode_a
...
!  to prevent tethers from slipping past each other:
   ...
   do n1=1,nnode_a-1
      ...
      do n2=n1+1,nnode_a
         ...
         dx=xnode_a(n1)-xnode_a(n2)
         dy=ynode_a(n1)-ynode_a(n2)
         dz=znode_a(n1)-znode_a(n2)

         if(dx*dx+dy*dy+dz*dz>d2) cycle

         do nf1=1,fanum

            if(a2node(nf1)/=n1) cycle

            do nf2=1,fanum

               if(a2node(nf2)/=n2) cycle

               npairnode_aa=npairnode_aa+1

               pairnode_aa(1,npairnode_aa)=nf1

               pairnode_aa(2,npairnode_aa)=nf2
