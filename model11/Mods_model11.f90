...
!==================================================
   ...
   subroutine setjob(nthreads,jobid,mbrad,mwid,rrad,rwid,rthick,falen,fanum,crlknum1,crlknum2, &
              myonum,kmemb,tension1,tension2,crlkorient,j_dep,cofilin,jmyoturn,jxldel,jxlturnover,halftime)

   implicit none

   integer nthreads,jobid,fanum,falen,crlknum1,crlknum2,myonum
   integer tension1,tension2,crlkorient,j_dep,cofilin,jmyoturn,jxldel,jxlturnover

   character (len=100) chara
   double precision mbrad,mwid,rrad,rwid,rthick,kmemb,halftime
...
!=========================================================
   ...
   subroutine makering(nbondwal,nwall,nmemb,nfa1,nfa2,fanum1,fanum2,nmyo,nbondmyo,myonum, &
               nlk,nbondlk,mybodylen,crlknum1,crlknum2,nxsol,nphisol,ntether, &
                astart,alen,a2mem,my2mem,filid,apos,lkstart,bondwal,bondmyo,mybody,myhead, &
                 bondlk,dxsol,dphisol,xboundmin,xboundmax,xwall,ywall,zwall, &
   ...
   integer,value::nbondwal,nwall,nmemb,nfa1,nfa2,fanum1,fanum2,nmyo,nbondmyo,myonum
   integer,value::nlk,nbondlk,mybodylen,crlknum1,crlknum2,nxsol,nphisol,ntether

   integer nact1,nact2,nfamax1,nfamax2,natom,natom_a1,natom_a2,nbond,crlknum,nbond0
   integer natom_a2m,nbond_a1,nbond_a2,nbond_a2m,fanum,nfa,crlknummax,crlknumactive
   ...
   integer,allocatable,dimension(:),intent(in)::astart,alen,a2mem,my2mem,filid,apos,lkstart
   integer,allocatable,dimension(:,:),intent(in)::bondwal,bondmyo,mybody,myhead,bondlk
...
!=========================================================
   subroutine writedcd(nframe,junit,natom,natom_a1,natom_a2,natom_a2m,nxsol,nphisol,nmemb, &
              fanum1,fanum2,nfa,nmyo,myonum,nlk,alen,astart,a2mem,my2mem,mybody,xfa,yfa,zfa,xmyo, &
              ymyo,zmyo,xlk,ylk,zlk,xmemb,ymemb,zmemb,xwall,ywall,zwall)
   ...
   integer,value::fanum1,fanum2,nfa,nmyo,myonum,nlk

   integer n,j,ja,jw,jm,jp,jx,jstart,jmemb

   integer,allocatable,intent(in),dimension(:)::alen,astart,a2mem,my2mem
...
!=========================================================
   ...
   subroutine getinfo(nstart,jfile,time0,runtime,natom,natom_a1,natom_a2,natom_a2m, &
              nwall,nxsol,nphisol,nmemb,nact1,nact2,nfamax1,nfamax2,nfa1,nfa2, &
   ...
   integer nmemb,nact1,nact2,nfamax1,nfamax2,nfa1,nfa2,fanum1,fanum2,ntether
...
!=========================================================
   ...
   subroutine ringin(nstart,nxsol,nphisol,nmemb,fanum,nfa,myonum,nmyo,mybodylen,crlknum, &
               nlk,astart,alen,a2mem,fa1stbound,my2mem,filid,apos,lkstart,lktyp,mybody, &
                myhead,mytyp,apar,fa2myo,fa2lk,xboundmin,xboundmax,dxsol,dphisol,xwall, &
                 ywall,zwall,xnorwall,ynorwall,znorwall,xfa,yfa,zfa,xmyo,ymyo,zmyo, &
   ...
   integer,value::nstart,nxsol,nphisol,nmemb,fanum,nfa,myonum,nmyo
   integer,value::mybodylen,crlknum,nlk
   integer n,jread(10)

   integer,allocatable,dimension(:)::astart,alen,a2mem,fa1stbound,my2mem,filid,apos,lkstart,lktyp
   integer,allocatable,dimension(:,:)::mybody,myhead,mytyp,apar
...
!=========================================================
   ...
   subroutine ringout(nstart,natom,natom_a1,natom_a2,natom_a2m,nwall,nxsol,nphisol,nmemb, &
               nact1,nact2,nfamax1,nfamax2,nfa1,nfa2,fanum1,fanum2,ntether,nfa,nmyo,myonum,nmyoturn, &
                mybodylen,nlk,crlknum1,crlknum2,crlknummax,crlknumactive,astart,alen,a2mem,my2mem, &
                 filid,apos,fa1stbound,lkstart,lktyp,mybody,myhead,mytyp,fa2myo,fa2lk,apar,dxsol,dphisol, &
                  xboundmin,xboundmax,xfa,yfa,zfa,xmyo,ymyo,zmyo,xlk,ylk,zlk, &
   ...
   integer nstart
   integer,value::natom,natom_a1,natom_a2,natom_a2m,nwall,nxsol,nphisol,nmemb
   ...
   integer,allocatable,intent(in),dimension(:)::astart,alen,a2mem,my2mem,filid,apos,fa1stbound,lkstart,lktyp
   integer,allocatable,intent(in),dimension(:,:)::mybody,myhead,mytyp,fa2myo,fa2lk,apar
...
!=========================================================
   ...
   subroutine my2memtether(myonum,mybody,my2mem,fxmemb,fymemb,fzmemb,fxmyo,fymyo,fzmyo, &
                xmemb,ymemb,zmemb,xmyo,ymyo,zmyo,kmy2mem,lmy2mem)
   ...
   integer,allocatable,dimension(:,:),intent(in)::mybody
...
!$omp shared(my2mem,mybody,kmy2mem,lmy2mem,l2on) &
!$omp private(dx,dy,dz,f,fx,fy,fz,dist,dist2) &

!$omp shared(myonum,xmemb,ymemb,zmemb) &
...
!=========================================================
   ...
   subroutine nodetether(nnode,nmb2node,memb2node,myonum,mybody,my2node, &
               fxnode,fynode,fznode,fxmemb,fymemb,fzmemb,fxmyo,fymyo,fzmyo, &
                xnode,ynode,znode,xmemb,ymemb,zmemb,xmyo,ymyo,zmyo,knode,lnode,lnode2)

   implicit none

   integer,value::nnode,nmb2node,myonum
   integer jnode,jmb,ja,jmy,j,nf,nm

   integer,allocatable,dimension(:),intent(in)::my2node
   integer,allocatable,dimension(:,:),intent(in)::memb2node,mybody
...
!=========================================================
   ...
   subroutine newactin(nmemb,myonum,crlknum,falen,nmono,jdir,nxsol,nphisol,ntether, &
               fanum,fanum1,fanum2,nfa,nfa1,nfa2,my2mem,jsursol,apar,fa1stbound,astart,alen, &
   ...
   integer,value::nmemb,myonum,crlknum,falen,nmono,jdir,nxsol,nphisol,ntether
...
!   integer,allocatable,dimension(:,:)::jfasoltem
   ...
   real(kind=8),value::ltether,l_a,l_mem,pi,p_tether!,xrmin,xrmax

   real(kind=8)::xmb,ymb,zmb!pcan(100),dsmall,dp,psum
...
!      if(ncan>100)then
!         print*,'why ring too thick?',na,jx,jp,proj
!         stop
!      end if

!      if(ncan<1) ncan=1

!      pcan(ncan)=pcan(ncan)+dp
...
!   distance=(ncan-1)*dsmall+r*dsmall
...
!      do n=fanum+1,fanum1+2,-1
      
!         astart(n)=astart(n-1)+length

!      end do

!      astart(fanum1+2:fanum+1)=astart(fanum1+1:fanum)+length
...
!      a2memtem(1:nfa2)=a2mem(nfa1+1:nfa)
...
!=========================================================
   ...
   subroutine myoturnover(nmyoturn,nfa,nmemb,myonum,mybodylen,nxsol,nphisol,mytyp,jmysol,mybody, &
               myhead,jsursol,jfasol,a2mem,mydist,my2mem,l_mb,l_mem,pi,pmyturn,dt,dphisol,xrmin,xrmax, &
                lmy2mem,xmyo,ymyo,zmyo,xmemb,ymemb,zmemb,xnorsurf,ynorsurf,znorsurf,xsurf,ysurf,zsurf)


   implicit none

   integer,value::nfa,nmemb,myonum,mybodylen,nxsol,nphisol

   integer nmyoturn

   integer jb,jh,nm,jx0,jp0,jm,jx,jp,j,jxget,jpget,j1,j2,na
...
!   bond(:,1:nbond)=pair_mb2(:,1:nbond)

!   open(1,file='test.psf')

!   write(1,21)nmemb,'!NATOM'

!   open(2,file='test.pdb')
...
!      WRITE(1,20)n,SEGID,RESNO,RES,TYP,tex,CHARG,MASS,0

!      write(2,42)tex,n,typ,res,ires,xmemb(n)/10,ymemb(n)/10,zmemb(n)/10,w1,w2,segid
...
!   end do
   ...
   write(junit2,2)runtime*0.000001,wallrate,natp,ringthick,ringwid,ringrad,2*nfa,a_conc,crlknumactiv
