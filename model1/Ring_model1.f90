...
!  For now we assume that linear rigidity of myosin is the same with that of F-actin
...
!  cell wall thickness = equilibrium distance b/w membrane and wall
...
!  if the bead size is l_mem then number of bead per hoop:
   ...
   nmeho=2*pi*mbrad/l_mem1

   dphi=2*pi/nmeho


   nhoop=(xboundmax-xboundmin+1.0d0)/l_mem1
...
!  total number of beads:
   ...
   do jh=1,nhoop
      ...
      x0=-(nhoop-1)/2*l_mem1+(jh-1)*l_mem1
...
!  number of myosins per node:

   allocate(nmy2node(nnode))
   nmy2node=0
...
!  unbound myosin turnover rate (node average myosin turnover rate ~ 14 seconds):
...
!  configuration of the system:
   ...
   allocate(xnode(nnode),ynode(nnode),znode(nnode),memb2node(nmb2node,nnode),nmy2node(nnode))
...
!  to prevent tethers from slipping past each other:
   allocate(pairnode_aa(2,fanummax*100),pairnode_mm(2,myonum*100))
...
!   lnode1=10.0d0 ! to bind to membrane
...
!  To write out trajectory
   ...
   nframe=0
         ...
         call writedcd(nframe,junit,natom,natom_a1,natom_a2,natom_a2n,nxsol,nphisol,nmemb,nnode, &
...
!        add new F-actin:
         ...
         if(nmono>falen.and.fanum1<fanummax1)then
            ...
            call newactin(myonum,crlknum,nmono,nxsol,nphisol,falen,nnode,nmb2node,jdir,fanum,fanum1,fanum2, &
...
!        myosin turnover:
         ...
         if(jmyoturn==1)then
            ...
            call myoturnover(nmyoturn,myonum,mybodylen,nnode,nxsol,nphisol,mytyp,jmysol,mybody,myhead, &
...
!        count neighbors:
         ...
         call neighbors(neinum5,mynei5,lknei5,xfa,yfa,zfa,astart,a2node,my2node,filid,alen,mybodylen, &
...
!        non-bond interaction: exclusion effect and coulomb
         ...
         call nodetether(nnode,nmb2node,memb2node,fanum,astart,a2node,myonum,mybody,my2node,fxnode,fynode, &
...
!------------------------------------------------
      ...
      if(runtime-oldtime>printtime)then
         ...
         if(nframe>=200)then
            ...
            call ringout(nstart,natom,natom_a1,natom_a2,natom_a2n,nwall,nxsol,nphisol,nmemb,nnode,nact1,nact2, &
