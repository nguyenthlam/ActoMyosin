module declare
   ...
   integer jprint,nrforce,jrforce,crlknum1,crlknum2,crlknum,nlk,lklen,nbondlk,nnode,nabound,ntether
   ...
   integer nxsol,nphisol,jh,jupdate,tension1,tension2,crlkorient,j_dep,cofilin,jxldel,jxlturnover
   integer jstart,astart0,jbreak,jmyoturn,nmyoturn
   integer natom,natom_a1,natom_a2,natom_a2n,nact,nfamax,fanummax,nmono,crlknummax,crlknumactive
   integer natp,jsignal,nmb2node,nturnover
   integer fanum1,fanum2,fanummax1,fanummax2,nfa1,nfa2,nfamax1,nfamax2,nact1,nact2
   ...
   integer,allocatable,dimension(:,:)::fa2myo,fa2lk,bondlk,apar,memb2node
   ...
   integer,allocatable,dimension(:,:)::pair5_my2,pair5_lk2,pair5_mb2,pairpart
   integer,allocatable,dimension(:,:)::pair_myac,pair_lkac,pair_mylk,pair_ac2,pair_my2,pair_lk2
   integer,allocatable,dimension(:,:)::pair_acmb,pair_mymb,pair_lkmb,pair_mb2,pairnode_aa,pairnode_mm
   ...
   integer,allocatable,dimension(:)::lkstart,lktyp,boundtyp,nmy2node

   double precision walrad,mbrad,rrad,rwid,rthick,mbrad2,invmbrad,mwid,kwall,lwall,node_size
   double precision l_lk1,l_lk2,l_lk,k_lk,klk_thet,thet_lk,phi_node_size,l_mem,k_mem,k_orient,kblock,kmemb,lsqueez,l_mem1
   double precision pi,k_a,l_a,invl_a,ka_thet,thet_fa,kmh_thet,kmb_thet,thet_mh1,thet_mh2,thet_mb,l_mh,l_mb
   double precision xh,yh,zh,xboundmin,xboundmax,xgap,prob,shift
   ...
   double precision delta,invdelta,beta,gam,fmaxfa2,fmaxmyo2,fmaxlk2,plk_bind,fmaxmb2,fmaxnode2
   double precision plk_ubind1,plk_ubind2,plk_off,k_scale,k_scale_mb,knode,lnode,lnode2,plk_turn
   double precision pmyturn,halftime
...
!   double precision,allocatable,dimension(:)::fxill_vdw,fyill_vdw,fzill_vdw
   ...
   double precision,allocatable,dimension(:)::xnode,ynode,znode
   double precision,allocatable,dimension(:)::fxnode,fynode,fznode
