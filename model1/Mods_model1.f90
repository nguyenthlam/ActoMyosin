...
!==================================================
   ...
   subroutine setjob(nthreads,jobid,mbrad,mwid,rrad,rwid,rthick,falen,fanum,myonum,crlknum1,crlknum2, &
              nnode,node_size,kmemb,tension1,tension2,crlkorient,j_dep,cofilin,jmyoturn,jxldel,jxlturnover,halftime)

   implicit none

   integer nthreads,jobid,fanum,falen,myonum,crlknum1,crlknum2,nnode
...
!=========================================================
   ...
   subroutine makering(nbondwal,nwall,nmemb,nnode,nfa1,nfa2,fanum1,fanum2,nmyo,nbondmyo,myonum,nlk, &
              nbondlk,mybodylen,crlknum1,crlknum2,nxsol,nphisol,nmb2node,astart,alen,a2node, &
              my2node,nmy2node,filid,apos,lkstart,bondwal,bondmyo,mybody,myhead,bondlk,memb2node,dxsol, &
   ...
   integer fanummax,fanummax1,fanummax2,natom_a2n,nbond_a2n,crlknum,nbond0,crlknummax,crlknumactive
   ...
   integer,allocatable,dimension(:),intent(in)::astart,alen,a2node,my2node,nmy2node,filid,apos,lkstart
...
!  to visualize bound actin

!   crlknum=crlknum1+crlknum2
!   nabound=2*(myonum+crlknum)
!   natom=natom+nabound
...
!      write(1,20)jatom,segid,resno,res,typ,tex,charg,mass,zero
...
!      bond(1,nbond)=bondfa(1,n)+nwall+nmemb
!      bond(2,nbond)=bondfa(2,n)+nwall+nmemb
...
!      write(2,42)tex,jatom,typ,res,ires,0.0,0.0,0.0,w1,w2,segid
...
!   write(1,*)xboundmin,xboundmax
   ...
   write(1,*)nnode
   do n=1,nnode
      write(1,*)memb2node(1:nmb2node,n),nmy2node(n)
   end do
   write(1,*)

!   nfahalf=2*nbondfa1
!   nfafull=2*nbondfa!1+nbondfa2)
...
!=========================================================
   subroutine writedcd(nframe,junit,natom,natom_a1,natom_a2,natom_a2n,nxsol,nphisol,nmemb,nnode, &
   ...
   integer nframe
   integer,value::junit,natom,natom_a1,natom_a2,natom_a2n,nxsol,nphisol,nmemb,nnode,fanum1,fanum2,nmyo,myonum,nlk
   integer n,j,ja,jw,jm,jp,jx,jstart,jnode

   integer,allocatable,intent(in),dimension(:)::alen,astart,a2node,my2node
...
!=========================================================
   ...
   subroutine getinfo(nstart,jfile,time0,runtime,natom,natom_a1,natom_a2,natom_a2n,nwall,nxsol,nphisol, &
               nmemb,nact1,nact2,nfamax1,nfamax2,nfa1,nfa2,fanum1,fanum2,fanummax1,fanummax2, &
   ...
   integer nmemb,nact1,nact2,nfamax1,nfamax2,nfa1,nfa2,fanum1,fanum2,fanummax1,fanummax2
...
!=========================================================
   ...
   subroutine ringin(nstart,nxsol,nphisol,nmemb,fanum,nfa,myonum,nmyo,nnode,mybodylen,crlknum, &
               nlk,nmb2node,astart,alen,a2node,my2node,nmy2node,filid,apos,lkstart,lktyp,mybody,myhead,mytyp,apar, &
   ...
   integer,allocatable,dimension(:)::astart,alen,a2node,my2node,nmy2node,filid,apos,lkstart,lktyp
...
13 read(1,*)chara
   ...
   do n=1,nnode
      read(1,*)memb2node(1:nmb2node,n),nmy2node(n)
   end do
...
!=========================================================
   ...
   subroutine ringout(nstart,natom,natom_a1,natom_a2,natom_a2n,nwall,nxsol,nphisol,nmemb,nnode,nact1,nact2, &
               nfamax1,nfamax2,nfa1,nfa2,nfa,fanummax1,fanummax2,fanum1,fanum2,fanum,nmyo,myonum,mybodylen, &
                nlk,crlknum1,crlknum2,crlknummax,crlknumactive,nmb2node,nmyoturn, &
                astart,alen,a2node,my2node,nmy2node,filid,apos,lkstart,lktyp,mybody,myhead,mytyp,fa2myo, &
                 fa2lk,apar,memb2node,dxsol,dphisol,xboundmin,xboundmax,xfa,yfa,zfa, &
                  xmyo,ymyo,zmyo,xnode,ynode,znode,xlk,ylk,zlk,xmemb,ymemb,zmemb, &
                   xsurf,ysurf,zsurf,xnorsurf,ynorsurf,znorsurf, &
   ...
   integer nstart
   integer,value::natom,natom_a1,natom_a2,natom_a2n,nwall,nxsol,nphisol,nmemb,nnode,nact1,nact2
   integer,value::nfamax1,nfamax2,nfa1,nfa2,nfa,fanummax1,fanummax2,fanum1,fanum2,fanum,nmyo,myonum,mybodylen
   integer,value::nlk,crlknum1,crlknum2,crlknummax,crlknumactive,nmb2node,nmyoturn

   integer n,crlknum

   integer,allocatable,intent(in),dimension(:)::astart,alen,a2node,my2node,nmy2node,filid,apos,lkstart,lktyp
...
!=========================================================
   ...
   subroutine neighbors(neinum5,mynei5,lknei5,xfa,yfa,zfa,astart,a2node,my2node,filid,alen,mybodylen, &
   ...
   integer,allocatable,intent(in),dimension(:)::astart,a2node,my2node,filid,alen,lkstart
...
!$omp shared(myonum,myhead,xfa,yfa,zfa,xmyo,ymyo,zmyo,filid,astart,a2node,my2node) &
...
!$omp shared(fa2lk,apar,lkstart,filid,lknei,lktyp) &
...
!=========================================================
   ...
   subroutine nodetether(nnode,nmb2node,memb2node,fanum,astart,a2node,myonum,mybody,my2node, &
               fxnode,fynode,fznode,fxmemb,fymemb,fzmemb,fxfa,fyfa,fzfa,fxmyo,fymyo,fzmyo, &
                xnode,ynode,znode,xmemb,ymemb,zmemb,xfa,yfa,zfa,xmyo,ymyo,zmyo,knode,lnode,lnode2, &
                 npairnode_aa,npairnode_mm,pairnode_aa,pairnode_mm)

   implicit none

   integer,value::nnode,fanum,myonum,nmb2node,npairnode_aa,npairnode_mm
   integer jnode,jmb,ja,jmy,j,nf,nm,n1,n2,nf1,nf2,ja1,ja2,nm1,nm2,jm1,jm2,j1,j2,n

   integer,allocatable,dimension(:),intent(in)::a2node,my2node,astart
   integer,allocatable,dimension(:,:),intent(in)::memb2node,mybody,pairnode_aa,pairnode_mm

   double precision,value::knode,lnode,lnode2
...
!$omp shared(a2node,my2node,astart,nmb2node,memb2node,mybody,knode,lnode,lnode2) &
...
!$omp shared(npairnode_aa,npairnode_mm,pairnode_aa,pairnode_mm,l2max,lmax) &
!$omp private(n1,n2,nf1,nf2,ja1,ja2,nm1,nm2,jm1,jm2,j1,j2,n) &
...
!$omp do schedule (guided,32)
   ...
   do nf=1,fanum
      ...
      dx=xnode(jnode)-xfa(ja)
      dy=ynode(jnode)-yfa(ja)
      dz=znode(jnode)-zfa(ja)
...
!$omp do
   ...
   do n=1,npairnode_aa
      ...
      dx1=0.1d0*(xfa(ja1)-xnode(n1))
      dy1=0.1d0*(yfa(ja1)-ynode(n1))
      dz1=0.1d0*(zfa(ja1)-znode(n1))

      dx2=0.1d0*(xfa(ja2)-xnode(n2))
      dy2=0.1d0*(yfa(ja2)-ynode(n2))
      dz2=0.1d0*(zfa(ja2)-znode(n2))
...
!=========================================================
   ...
   subroutine rforceset(jrforce,nrforce,nmemb,nfa,nmyo,nlk,pi,rxmemb,rymemb,rzmemb, &
              rxfa,ryfa,rzfa,rxmyo,rymyo,rzmyo,rxlk,rylk,rzlk,k_scale,k_scale_mb)
   ...
   double precision,value::pi,k_scale,k_scale_mb
...
!$omp shared(nrforce,nmax,nh1,nh2,nmemb,nfa,nmyo,nlk,pi,k_scale,k_scale_mb) &
...
!      iseed=seed/127773+j*539
      ...
      rxfa(1:nfa,j)=k_scale*rx(n+1:n+nfa)
      ryfa(1:nfa,j)=k_scale*ry(n+1:n+nfa)
      rzfa(1:nfa,j)=k_scale*rz(n+1:n+nfa)

      n=n+nfa

      rxmyo(1:nmyo,j)=k_scale*rx(n+1:n+nmyo)
      rymyo(1:nmyo,j)=k_scale*ry(n+1:n+nmyo)
      rzmyo(1:nmyo,j)=k_scale*rz(n+1:n+nmyo)

      n=n+nmyo

      rxlk(1:nlk,j)=k_scale*rx(n+1:n+nlk)
      rylk(1:nlk,j)=k_scale*ry(n+1:n+nlk)
      rzlk(1:nlk,j)=k_scale*rz(n+1:n+nlk)
...
!=========================================================
   ...
   subroutine newactin(myonum,crlknum,nmono,nxsol,nphisol,falen,nnode,nmb2node,jdir,fanum,fanum1,fanum2, &
               nfa,nfa1,nfa2,jsursol,memb2node,apar,jfasol,astart,alen,fadist,apos,filid,a2node, &
                fa2myo,fa2lk,l_a,l_mem,pi,lnode,xfa,yfa,zfa,xmemb,ymemb,zmemb, &
                 xnorsurf,ynorsurf,znorsurf,xsurf,ysurf,zsurf)

   implicit none

   integer,value::myonum,crlknum,nmono,nxsol,nphisol,falen,nnode,nmb2node,jdir
   integer fanum,fanum1,fanum2,nfa,nfa1,nfa2

   integer length,na,nnew,nm,jm,jmax,jx0,jp0,j,jx,jp,jxget,jpget,j1,j2,n,jnode,nl,jl
...
!=========================================================
   ...
   subroutine myoturnover(nmyoturn,myonum,mybodylen,nnode,nxsol,nphisol,mytyp,jmysol,mybody,myhead, &
               nmy2node,my2node,mydist,pmyturn,dt,dphisol,dxsol,xmyo,ymyo,zmyo,xnode,ynode,znode)

   implicit none

   integer,value::myonum,mybodylen,nnode,nxsol,nphisol

   integer nmyoturn

   integer jb,jh,nm,jnode0,jnode,j,djx,djp

   integer,allocatable,dimension(:,:)::mytyp,jmysol

   integer,allocatable,dimension(:,:),intent(in)::mybody,myhead

   integer,allocatable,dimension(:)::nmy2node,my2node,mydist

   real(kind=8),value::pmyturn,dt,dphisol,dxsol

   real(kind=8)::r,dxt,dyt,dzt,rad,cos_t0,sin_t0,cos_t,sin_t,cos_t1,sin_t1,t1,dy,dz,dy1,dz1

   real(kind=8),allocatable,dimension(:)::xmyo,ymyo,zmyo

   real(kind=8),allocatable,dimension(:),intent(in)::xnode,ynode,znode



!$omp parallel &
!$omp default(none) &
!$omp private(jb,jh,nm,jnode0,jnode,j,djx,djp) &
!$omp private(r,dxt,dyt,dzt,rad,cos_t0,sin_t0,cos_t,sin_t,cos_t1,sin_t1,t1,dy,dz,dy1,dz1) &
!$omp shared(myonum,mybodylen,nnode,nxsol,nphisol) &
!$omp shared(mytyp,jmysol,mybody,myhead,nmy2node,my2node,mydist,pmyturn,dt,dphisol,dxsol) &
!$omp shared(xmyo,ymyo,zmyo,xnode,ynode,znode) &
...
12    call random_number(r)
      ...
      if(nmy2node(jnode)>=30.or.jnode==jnode0) goto 12

!$omp atomic
      nmy2node(jnode)=nmy2node(jnode)+1

!$omp atomic
      nmy2node(jnode0)=nmy2node(jnode0)-1

      my2node(nm)=jnode

!     translation vector:

      dxt=xnode(jnode)-xnode(jnode0)
      dyt=ynode(jnode)-ynode(jnode0)
      dzt=znode(jnode)-znode(jnode0)

!     translation of the solid angle index along x:
      djx=dxt/dxsol

!     rotation matrix (2D on yz plane)

      rad=sqrt(ynode(jnode0)*ynode(jnode0)+znode(jnode0)*znode(jnode0))

      cos_t0=ynode(jnode0)/rad
      sin_t0=znode(jnode0)/rad

      rad=sqrt(ynode(jnode)*ynode(jnode)+znode(jnode)*znode(jnode))

      cos_t=ynode(jnode)/rad
      sin_t=znode(jnode)/rad

      cos_t1=cos_t*cos_t0+sin_t*sin_t0
      sin_t1=sin_t*cos_t0-cos_t*sin_t0

      t1=acos(cos_t1)

      if(sin_t1<0.0d0) t1=-t1

!     translation of the solid angle index circumferentially
      djp=t1/dphisol
...
!     move the heads:
      ...
      do jh=1,2
         ...
         dy=ymyo(j)-ynode(jnode0)
         dz=zmyo(j)-znode(jnode0)

         dy1=cos_t1*dy-sin_t1*dz-dy
         dz1=sin_t1*dy+cos_t1*dz-dz

         xmyo(j)=xmyo(j)+dxt
         ymyo(j)=ymyo(j)+dyt+dy1
         zmyo(j)=zmyo(j)+dzt+dz1

         jmysol(1,j)=jmysol(1,j)+djx

         if(jmysol(1,j)<1) jmysol(1,j)=jmysol(1,j)+nxsol

         if(jmysol(1,j)>nxsol) jmysol(1,j)=jmysol(1,j)-nxsol

         jmysol(2,j)=jmysol(2,j)+djp

         if(jmysol(2,j)<1) jmysol(2,j)=jmysol(2,j)+nphisol

         if(jmysol(2,j>nphisol)) jmysol(2,j)=jmysol(2,j)-nphisol
...
!=========================================================
   ...
   if(crlknummax>=crlknumactive) return
...
!============================================================================
                  ...
                  npair_ac2,pair_ac2,npair_my2,pair_my2,npair_lk2,pair_lk2, &
                   npair5_mb2,pair5_mb2,npair_mb2,pair_mb2,r_off,l_pair,thet2by2, &
                    pairpart,boundtyp,l_mem,xboundmin,xboundmax,shift, &
                     nnode,xnode,ynode,znode,fanum,a2node,myonum,my2node, &
                      npairnode_aa,npairnode_mm,pairnode_aa,pairnode_mm)
   ...
   integer n1,n2,n3,n4,jexit,j1,j2,j3,j4,m1,m2,jp,np,nf1,nf2,nm1,nm2
   ...
   integer,allocatable,intent(in),dimension(:)::a2node,my2node

   integer,allocatable,dimension(:,:)::pair_myac,pair_lkac,pair_mylk,pair_ac2,pair_my2,pair_lk2
   integer,allocatable,dimension(:,:)::pair_mb2,pairpart,pairnode_aa,pairnode_mm
...
!$omp shared(xmemb,ymemb,zmemb,dist,pair_mb2,delta,low,up,up1,l_mem,xsum,ysum,zsum) &
   ...
   do n1=1,nnode-1
      ...
      do n2=n1+1,nnode
         ...
         do nm1=1,myonum
            ...
            if(my2node(nm1)/=n1) cycle

            do nm2=1,myonum

               if(my2node(nm2)/=n2) cycle
   ...
   subroutine constraints(nfa,nmemb,nmyo,nlk,jupdate,nxsol,nphisol,nthreads, &
               fadist,mydist,lkdist,jmbsol,jsursol,jfasol,jmysol,jlksol,nsurf, &
                kwall,lsqueez,wthick,k_mem,l_mem,xwall,ywall,zwall,xnorwall, &
                 ynorwall,znorwall,xmemb,ymemb,zmemb,xboundmin,xboundmax,xfa,yfa,zfa, &
...
!   integer,allocatable,intent(in),dimension(:)::a2mem
   ...
   double precision,value::kwall,lsqueez,wthick,k_mem,l_mem,xboundmin,xboundmax
...
!$omp shared(xmemb,ymemb,zmemb,fxmemb,fymemb,fzmemb,xboundmin,xboundmax) &
...
!     blocking from the x boundaries:
      ...
      if(xfa(n)<xboundmin)then
         fxfa(n)=fxfa(n)+xboundmin-xfa(n)
      else if(xfa(n)>xboundmax)then
         fxfa(n)=fxfa(n)+xboundmax-xfa(n)
      end if
...
!     blocking from the x boundaries:
      ...
      if(xmyo(n)<xboundmin)then
         fxmyo(n)=fxmyo(n)+xboundmin-xmyo(n)
      else if(xmyo(n)>xboundmax)then
         fxmyo(n)=fxmyo(n)+xboundmax-xmyo(n)
      end if
...
!     blocking from the x boundaries:
      ...
      if(xlk(n)<xboundmin)then
         fxlk(n)=fxlk(n)+xboundmin-xlk(n)
      else if(xlk(n)>xboundmax)then
         fxlk(n)=fxlk(n)+xboundmax-xlk(n)
      end if
...
!   integer,allocatable,dimension(:),intent(in)::lktyp
   ...
   real(kind=8)::e_tk(4),e_tk2(4),e_wid(4),e_wid2(4),ringrad,rad2,rad,ringwid,a_conc,ringvol,thet,y0,z0
