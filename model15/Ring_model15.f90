...
13    call random_number(r)
      ...
      xmyo(nmyo)=-rwid/2+rwid*r!xnode(jnode)+node_size*(r-0.5d0)/2

!     pick the angle of first body bead:

      call random_number(r)

      phi0=2*pi*r!phi_node(jnode)+phi_node_size*(r-0.5d0)/2

!     pick the "radius" of the myosin 1st body bead

      call random_number(r)

      rad=mbrad!mbrad-node_size*r/4
...
!     direction of the myosin filament:
      ...
      dy=y0-ymyo(nmyo)!ynode(jnode)
      dz=z0-zmyo(nmyo)!znode(jnode)
...
!  write out the ring:
   ...
   call makering(nbondwal,nwall,nmemb,nfa1,nfa2,fanum1,fanum2,nmyo,nbondmyo,myonum, &
...
!  crosslinker type to tell if it is deleted:
   ...
   call ringin(nstart,nxsol,nphisol,nmemb,fanum,nfa,myonum,nmyo,mybodylen,crlknum, &
...
!------------------------------------------------------------
   ...
   nframe=0
         ...
         call writedcd(nframe,junit,natom,natom_a1,natom_a2,nxsol,nphisol,nmemb, &
...
!        myosin turnover:
         ...
         if(jmyoturn==1)then
            ...
            call myoturnover(nmyoturn,nmemb,myonum,mybodylen,nxsol,nphisol,mytyp,jmysol,mybody, &
...
!        non-bond interaction: exclusion effect and coulomb
         ...
         call my2memtether(myonum,mybody,my2mem,fxmemb,fymemb,fzmemb,fxmyorep,fymyorep, &
              fzmyorep,xmemb,ymemb,zmemb,xmyo,ymyo,zmyo,kmy2mem,lmy2mem)
...
!------------------------------------------------
      ...
      if(runtime-oldtime>printtime)then
         ...
         if(nframe>=200)then
            ...
            call ringout(nstart,natom,natom_a1,natom_a2,nwall,nxsol,nphisol,nmemb, &
            ...
            if(runtime*0.000001>110.0) exit
