...
!==================================================
   ...
   integer nthreads,jobid,fanum,falen,myonum,crlknum1,crlknum2,ntether
   integer tension1,tension2,crlkorient,j_dep,cofilin,jmyoturn,jxldel,jxlturnover

   character (len=100) chara
   double precision mbrad,mwid,rrad,rwid,rthick,kmemb,ltether,halftime
...
!=========================================================
   ...
   subroutine makering(nwall,nxsol,nphisol,nwalsurf,nwalrad,nmemb,fanum1,fanum2,nfa1,nfa2, &
   ...
   integer,value::ntether,myonum,mybodylen,nmyo,nbondmyo
   integer,value::crlknum1,crlknum2,lklen,nlk,nbondlk

   integer natom,natom_a1,natom_a2,natom_a2m,nact1,nact2,nfamax1,nfamax2,nmyoturn

   integer fanum,nfa,nbond_a1,nbond_a2,nbond_a2m,nbond0
   ...
   integer,allocatable,dimension(:),intent(in)::astart,alen,filid,a2mem,apos,lkstart
   integer,allocatable,dimension(:)::fa1stbound,waltyp,lktyp
   integer,allocatable,dimension(:,:),intent(in)::mybody,myhead,bondmyo,bondlk,wposid
   integer,allocatable,dimension(:,:)::bond,mytyp,apar
...
!   nbond=nbondfa1+nbondfa2+nbondmyo+nbondlk
...
!   nwalrep=2*(nwall+nwrad)

!   allocate(xwalrep(nwalrep),ywalrep(nwalrep),zwalrep(nwalrep))
...
!               xwalrep(n)=xwall(jp,jx)
!               ywalrep(n)=ywall(jp,jx)
!               zwalrep(n)=zwall(jp,jx)
...
!+2*nbondfa1+2*nbondfa2+nmyo+nlk

!  to visualize bound actin

!   crlknum=crlknum1+crlknum2
!   nabound=2*(2*myonum+crlknum)
!   natom=natom+nabound

!  to visualize membrane tethering:
...
!      write(1,*)alen(n),fa1stbound(n),astart(n)
!   enddo
   write(1,*)
   ...
   allocate(mytyp(4,myonum),fa2myo(4,myonum))
   mytyp=1; fa2myo=0

   do n=1,myonum
      write(1,*)myhead(1:4,n),mybody(1:mybodylen,n)
      write(1,*)mytyp(1:4,n),fa2myo(1:4,n)
   end do
   write(1,*)
...
!=========================================================
   ...
   integer,value::nmemb,fanum1,fanum2,natom_a1,natom_a2,nfa,natom_a2m,nmyo,nlk
   integer nframe
   integer n,j,jw,jm,jp,jx,jstart

   integer,allocatable,intent(in),dimension(:)::alen,a2mem,astart
   integer,allocatable,dimension(:)::waltyp
   integer,allocatable,intent(in),dimension(:,:)::wposid
...
!if(jstart+j-1==1515)then
!print*,'jw',jw-1
!end if

!if(jstart+j==1515)then
!print*,'jw next',jw
!end if
...
!   xw(jw+1:jw+nabound)=0.0
!   yw(jw+1:jw+nabound)=0.0
!   zw(jw+1:jw+nabound)=0.0

!   jb=0

!   do n=1,nfa
!      if(apar(1,n)>0.and.apar(2,n)/=0)then
!         jb=jb+1
!         xw(jw+jb)=0.1*xfa(n)
!         yw(jw+jb)=0.1*yfa(n)
!         zw(jw+jb)=0.1*zfa(n)
...
!   xw(jw+1:jw+2*ntether)=0.0
!   yw(jw+1:jw+2*ntether)=0.0
!   zw(jw+1:jw+2*ntether)=0.0

!open(15,file='check2.txt')
!do n=1,nfa
!write(15,*)n,a2mem(n)
!end do
!close(15)
...
!=========================================================
   ...
   integer natom,natom_a1,natom_a2,natom_a2m,nwall,nxsol,nphisol,nwalsurf,nwalrad
...
!=========================================================
   ...
   subroutine ringin(nstart,nxsol,nphisol,nmemb,fanum,nfa,myonum,nmyo,nwalrad,nwalsurf,nwall, &
   ...
   integer,value::nstart,nxsol,nphisol,nmemb,fanum,nfa,myonum,nmyo,nwalrad,nwalsurf,nwall
   integer,value::mybodylen,crlknum,nlk

   integer n,jread(1:20)

   integer,allocatable,dimension(:)::astart,alen,filid,a2mem,apos,fa1stbound,waltyp,lkstart,lktyp
   integer,allocatable,dimension(:,:)::mybody,myhead,mytyp,apar,wposid
   integer,allocatable,dimension(:,:)::fa2myo,fa2lk
...
!print*,'filid',filid(1:6)

!open(15,file='check3.txt')
!do n=1,nfa
!write(15,*)n,a2mem(n)
!end do
!close(15)
...
5  read(1,*)chara
   ...
   do n=1,myonum
      read(1,*)myhead(1:4,n),mybody(1:mybodylen,n)
      read(1,*)mytyp(1:4,n),fa2myo(1:4,n)
   end do
...
!=========================================================
   ...
   subroutine ringout(nstart,natom,natom_a1,natom_a2,natom_a2m,nwall,nxsol,nphisol,nwalsurf, &
   ...
   integer,value::natom,natom_a1,natom_a2,natom_a2m
   ...
   integer,value::nfa,nmyo,myonum,mybodylen,nlk,crlknum1,crlknum2,crlknummax,crlknumactive,nmyoturn

   integer lentemp,n,fanum,crlknum

   integer,allocatable,intent(in),dimension(:)::astart,alen,filid,a2mem,apos,fa1stbound,waltyp,lkstart,lktyp
   integer,allocatable,intent(in),dimension(:,:)::mybody,myhead,mytyp,fa2myo,fa2lk,apar,wposid
...
!      jtem0(tid)=jrand+4*myonum+tid-1
...
!$omp parallel &
!$omp default(none) &
!$omp private(nm,jh,jtem,jmyo,ncan,jn,ja,jf,j,jexit,acan,pcan,j1,j2) &
!$omp private(d2max,dx,dy,dz,d2,psum,rtem,pact,ratio,iseed) &
!$omp shared(myonum,neinum,tension2,nrand,rat0,normal) &
...
!$omp shared(rands,xmyo,ymyo,zmyo,xfa,yfa,zfa,invmylen2,mybody,mybodylen,seed) &
!$omp reduction(+:natp)
...
!   jraind=maxval(jtem0(1:nthreads))!jrand+2*myonum

   jrand=jrand+4*myonum
...
!$omp shared(jrand,nrand,crlknum,crlknum1,lklen,neinum) &
!$omp shared(fa2lk,apar,lkstart,lktyp,filid,astart,apos,fa1stbound,lknei,seed) &
...
!=========================================================
   ...
   integer length,na,nnew,nm,jm,nl,jl,jx0,jp0,jtether,j,npick,jx,jp,jxget,jpget,j1,j2,nsum,n,limit
...
!   real(kind=8)::distmin,distmax!,pcan(100),dsmall,dp,psum
   ...
   real(kind=8)::dist2,d2,dx,dy,dz,xn,yn,zn,proj,proj0,newdist
...
!  total tethering of actin to membrane:
   ...
   limit=p_tether*nfa
   limit=min(limit,ntether)

!  distance away from membrane:

   distance=(x0-xmb)*xn0+(y0-ymb)*yn0+(z0-zmb)*zn0
...
!   call random_number(r)
   ...
   if(distance<ltether)then

      call random_number(r)

      newdist=ltether*r

      x0=x0-xn0*(distance-newdist)
      y0=y0-yn0*(distance-newdist)
      z0=z0-zn0*(distance-newdist)
...
!   distance=(distmax-distmin)*r+distmin
   ...
   if(distance<ltether.and.nsum<limit)then
      jtether=1
...
!            ja=jstart+j-1     ! used to be afil(j,n)
...
!      if(jstop>=length)then
!         cycle
!      end if


!      ja=jstart+length-1     ! used to be afil(j,n)
...
!=========================================================
   ...
   subroutine myoturnover(nmyoturn,nfa,nmemb,myonum,mybodylen,nxsol,nphisol,mytyp,jmysol,mybody,myhead, &
   ...
   real(kind=8),value::l_mb,l_mem,pi,pmyturn,dt,dphisol
!   real(kind=8),value::xrmin,xrmax,distmin,distmax
   ...
   real(kind=8),allocatable,dimension(:)::xmyo,ymyo,zmyo,xfa,yfa,zfa
   real(kind=8),allocatable,dimension(:),intent(in)::xmemb,ymemb,zmemb
   real(kind=8),allocatable,dimension(:,:),intent(in)::xnorsurf,ynorsurf,znorsurf,xsurf,ysurf,zsurf
...
!$omp shared(nfa,nmemb,myonum,mybodylen,nxsol,nphisol) &
!$omp shared(mytyp,jmysol,mybody,myhead,jsursol,jfasol,mydist) &
!$omp shared(l_mb,l_mem,pi,pmyturn,dt,dphisol) &
!$omp shared(xmyo,ymyo,zmyo,xfa,yfa,zfa,xmemb,ymemb,zmemb) &
...
!$omp do
   ...
   do nm=1,myonum
      ...
      if(mytyp(1,nm)/=2.or.mytyp(2,nm)/=2.or.mytyp(3,nm)/=2.or.mytyp(4,nm)/=2) cycle
...
!     now assign coordinates for the body:
      ...
      do j=1,mybodylen
         ...
         if(j==mybodylen)then
            ...
            jmysol(1,myhead(3:4,nm))=jx0

            jmysol(2,myhead(3:4,nm))=jp0
...
!        update coordinates for the next bead:
         ...
         if(jxget==0)then
            print*,'error in jxget for myosin',jx0,jp0
            stop
         end if
...
!$omp end parallel
   ...
   do jm=1,nmyo-4-mybodylen ! the last myosin beads don't have pairs
...
!     force on membrane from turgor pressure:

!      dist=sqrt(ymemb(n)*ymemb(n)+zmemb(n)*zmemb(n))

!      invdist=1.0d0/dist

!      fxmemb(n)=fxmemb(n)-fturgor*xn
!      fymemb(n)=fymemb(n)-fturgor*yn
!      fzmemb(n)=fzmemb(n)-fturgor*zn

!      fymemb(n)=fymemb(n)+fturgor*ymemb(n)*invdist
!      fzmemb(n)=fzmemb(n)+fturgor*zmemb(n)*invdist


!     blocking from cell wall:
...
!      if(dist+lwall>rwall(jp,jx))then
!         f=kwall*(rwall(jp,jx)-dist-lwall)*invdist
!         fymemb(n)=fymemb(n)+f*ymemb(n)!*invdist
!         fzmemb(n)=fzmemb(n)+f*zmemb(n)!*invdist
!      end if
...
!         if(dist+l_mem>rmemb(jp,jx))then


!            f=k_mem*(rmemb(jp,jx)-dist-l_mem)/dist
!            fyfa(n)=fyfa(n)+f*yfa(n)!*invdist
!            fzfa(n)=fzfa(n)+f*zfa(n)!*invdist
...
!            if(dist+l_mem+l_memby2>rmemb(jp,jx))then
!               fadist(n)=1
!            else
!               fadist(n)=0
!            end if
...
!         dist=sqrt(ymyo(n)*ymyo(n)+zmyo(n)*zmyo(n))

!         if(dist+l_mem>rmemb(jp,jx))then


!            f=k_mem*(rmemb(jp,jx)-dist-l_mem)/dist
!            fymyo(n)=fymyo(n)+f*ymyo(n)!*invdist
!            fzmyo(n)=fzmyo(n)+f*zmyo(n)!*invdist
...
!            f=k_mem*(rmemb(jp,jx)-dist-l_mem)/dist
!            fylk(n)=fylk(n)+f*ylk(n)!*invdist
!            fzlk(n)=fzlk(n)+f*zlk(n)!*invdist
