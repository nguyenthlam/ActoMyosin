...
!   Select job:
   call setjob(nthreads,jobid,mbrad,mwid,rrad,rwid,rthick,falen,fanum,crlknum1,crlknum2, &
        myonum,nnode_a,node_size,kmemb,tension1,tension2,crlkorient,j_dep,jmyoturn,cofilin,jxldel,jxlturnover,halftime)
...
17    call random_number(r)
      ...
      phi_node_my(n)=n*dphi+dphi*(r-0.5d0)/2

      call random_number(r)

      xnode_my(n)=rwid*(r-0.5d0)/2

      ynode_my(n)=mbrad*cos(phi_node_my(n))

      znode_my(n)=mbrad*sin(phi_node_my(n))

      jm=0

      do nm=1,nmemb

         if(jm==nmb2node_my)then
            exit
         end if

         if(mark(nm)==1) cycle

         dx=xnode_my(n)-xmemb(nm)
         dy=ynode_my(n)-ymemb(nm)
         dz=znode_my(n)-zmemb(nm)

         if(dx*dx+dy*dy+dz*dz<node_size*node_size/4)then
            jm=jm+1
            memb2node_my(jm,n)=nm
            mark(nm)=1
         end if
...
!     pick the x position of the 1st body bead:
      ...
      xmyo(nmyo)=xnode_my(jnode)+node_size*(r-0.5d0)/2

!     pick the angle of first body bead:

      call random_number(r)

      phi0=phi_node_my(jnode)+phi_node_size*(r-0.5d0)/2
...
!     direction of the myosin filament:
      ...
      dy=y0-ynode_my(jnode)
      dz=z0-znode_my(jnode)
...
!  write out the ring:
   ...
   call makering(nbondwal,nwall,nmemb,nnode_a,nnode_my,nfa1,nfa2,fanum1,fanum2,nmyo,nbondmyo,myonum,nlk, &
...
!  crosslinker type to tell if it is deleted:
   ...
   call ringin(nstart,nxsol,nphisol,nmemb,fanum,nfa,myonum,nmyo,nnode_a,nnode_my,mybodylen,crlknum, &
...
!  forces:
   ...
   allocate(fxnode_my(nnode_my),fynode_my(nnode_my),fznode_my(nnode_my))
...
!------------------------------------------------------------
   ...
   nframe=0
         ...
         call writedcd(nframe,junit,natom,natom_a1,natom_a2,natom_a2n,nxsol,nphisol,nmemb,nnode_a,nnode_my, &
...
!        depolymerization of F-actin:
         ...
         if(jmyoturn==1)then
            ...
            call myoturnover(nmyoturn,nnode_a,nmb2node_a,nnode_my,nmb2node_my,nmemb,mybodylen,nxsol,nphisol,mytyp,jmysol, &
               memb2node_a,memb2node_my,mybody,myhead,jsursol,jfasol,mydist,l_mb,l_mem,pi,pmyturn,dt,dphisol,xrmin,xrmax, &
...
!        non-bond interaction: exclusion effect and coulomb
         ...
         call nodetether(nnode_a,nnode_my,nmb2node_a,nmb2node_my,memb2node_a,memb2node_my,fanum, &
              astart,a2node,myonum,mybody,my2node,fxnode_a,fynode_a,fznode_a,fxnode_my,fynode_my, &
              fznode_my,fxmemb,fymemb,fzmemb,fxfarep,fyfarep,fzfarep,fxmyorep,fymyorep,fzmyorep, &
              xnode_a,ynode_a,znode_a,xnode_my,ynode_my,znode_my,xmemb,ymemb,zmemb, &
              xfa,yfa,zfa,xmyo,ymyo,zmyo,knode,lnode,lnode2, &
...
!     update coordinates:
      ...
      if(jforce1==1)then
         ...
         fmaxnode_my2=maxval(fxnode_my*fxnode_my+fynode_my*fynode_my+fznode_my*fznode_my)
      ...
      xnode_my=xnode_my+gam*fxnode_my
      ynode_my=ynode_my+gam*fynode_my
      znode_my=znode_my+gam*fznode_my
...
!------------------------------------------------
      ...
      if(runtime-oldtime>printtime)then
         ...
         if(nframe>=200)then
            ...
            call ringout(nstart,natom,natom_a1,natom_a2,natom_a2n,nwall,nxsol,nphisol,nmemb,nnode_a,nnode_my,nact1,nact2, &
